...
func main() {
	...
	tag, err := pool.Exec(ctx, "UPDATE users SET is_admin = false, updated_at = now() WHERE email = $1", email)
	if err != nil {
	...
	fmt.Printf("User %s demoted from admin.\n", email)
}
...
func confirm() bool {
	...
	fmt.Printf("Type %s to confirm admin demotion: ", code)
